...
*/
	...
	public function actionRecent($id)
	{
		...
		$details=Sales::model()->findAll('transaction_id=:k and shop_id=:g',array(':k'=>$id,':g'=>Yii::app()->user->shop_id));
...
$lid2 = Sales::model()->find($criteria1);
...
try {
   ...
   // $printer -> text($conf->company_name."\n");
    ...
    $printer -> text("Served by: ".User::model()->find('id=:u',array(':u'=>Yii::app()->user->id))->username."\n");
	...
	$printer -> text("Table No: ".$positions[0]->table_number."\n\n");
					...
					$tran="Receipt ID: ".$transid."\n";	
					}else{
						$tran="Captain Order NO: ".$transid." \n";
					}
    ...
    if($positions[0]->closed==1){
					 ...
					 $printer -> text("RECEIPT ID: ".$transid."\n\n");
						...
						$printer -> text("Captain Order NO: ".$transid."\n\n");
					}
    ...
    foreach($positions as $position){
				...
				$printer -> text($content."\n");
					    ...
					    foreach($positions as $position){
					    	$cat=Sales::model()->findAll('category=:j and created_at=:g',array(':j'=>$position->category,':g'=>$time));
					    	foreach($cat as $ca){
...
alert('One or more of the items on your cart is out of stock!');
			...
			// if($sale->type==2){
			// 	// put the code here for whole sale
			// }
...
// if we impliment wholesale
			...
			{
				$linked=Sales::model()->findAll('transaction_id=:h',array(':h'=>Yii::app()->session['tid']));
				...
				if(empty($position->supplier))
				{
					...
					$sale->tax=$tax;
					$sale->balance=0;
					$sale->tendered=$tendered+$ltotal;
					$sale->supplier=$position->supplier;
					$sale->supply_price=$position->supply_price;
					$sale->item_discount=$position->single_item_discount;
					$sale->shop_id=Yii::app()->user->shop_id;
					$sale->amount_paid=$dtender+$ltotal;
					$sale->sale_balance=$total-$dtender-$disco-$disc;
					$sale->closed=0;
