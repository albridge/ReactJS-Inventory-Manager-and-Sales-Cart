...
class InventoryController extends Controller
{
	...
	public function accessRules()
	{
		...
		return array(
		array('allow',
			'actions'=>array('kitmenu','kitmenu2','sendlogin','salesapi'),
			'users'=>array('*'),
		),
		array('allow',
...
try {
    ...
    foreach($positions as $pam)
    {

    	$pastor=Salesx::model()->findAll('broad=:h and created_at=:d',array(':h'=>$pam->broad,':d'=>$time));
		$printer=Categories::model()->find('id=:u',array(':u'=>$pam->category));
		$dprinter=Printers::model()->findByPk($printer->printer)->printer_name;
   ...
   // $connector = "mark";
	//$connector = new WindowsPrintConnector("taire");
	 $connector = new WindowsPrintConnector($dprinter);

	// $connector = new NetworkPrintConnector("192.168.43.58",9100);

	// $connector = new WindowsPrintConnector( "smb://mercy-PC/mark");
	// $connector = new WindowsPrintConnector("smb://mercy:kekeluv@mercy-PC/mark");
    ...
    $total=0;
					   ...
					   // foreach($positions as $position){
					    	$cat=Salesx::model()->findAll('created_at=:g and broad=:k',array(':g'=>$time,':k'=>$broad[0]->category));
					    	foreach($pastor as $ca){
...
// if we impliment wholesale
			...
			// }
				...
				if(empty($position->supplier))
				{
					...
					$broad=Categories::model()->find('id=:j',array(':j'=>$position->category))->broad;
					$sale->broad=$broad;
...
/*
		...
		*/
		...
		public function actionReports()
		{
			...
			if(isset($_GET['Inventory']))
				...
				$one=$_GET['Inventory']['date1'];
					// $dyear=$model->created_at;
				$two=$_GET['Inventory']['date2'];

			
			$dyear=$_GET['Inventory']['date1'].' 13';
					// $dyear=$model->created_at;
			$date2=$_GET['Inventory']['date2'].' 12';
    ...
    // something here o as in put your code here
   	...
   	if($shop=='5000')
   	{// and substr(tame,1,2) < 14 and substr(tame,1,2) > 7
// the code for all location goes here
   		$sales_sql="select transaction_id, tendered, amount_paid, table_number, item_id, item_name, unit_price, supply_price, qty, total, customer_id, staff, payment_type, tax, discount, id, balance, shop_id from sales where (substr(created_at,1,13) between '".$dyear."' and '".$date2."') and closed=1 group by transaction_id order by id desc";
   		$sql1="select item_name, sum(qty) as nu, discount from sales where created_at between '".$dyear."' and '".$date2."' and closed=1 group by item_id order by nu desc limit 10";
   		$cust_sql="select * from sales where  created_at between '".$dyear."' and '".$date2."' and closed=1 group by transaction_id";
   	}elseif(!empty($ptype)){
   	  
   // code for specific shop location goes here
   		$sales_sql="select transaction_id, tendered, amount_paid, item_id, item_name, unit_price, supply_price, qty, total, customer_id, staff, payment_type, table_number, tax, discount, id, balance, shop_id from sales where created_at between '".$dyear."' and '".$date2."' and staff='".$shop."' and payment_type='".$ptype."' and closed=1 group by transaction_id order by id desc";
   		...
   		$cust_sql="select * from sales where  created_at between '".$dyear."' and '".$date2."' and staff='".$shop."' and payment_type='".$ptype."' and closed=1 group by transaction_id";
 
}elseif(empty($ptype)){
   // code for specific shop location goes here
   		$sales_sql="select transaction_id, tendered, amount_paid, item_id, item_name, unit_price, supply_price, qty, total, customer_id, staff, payment_type, table_number, tax, discount, id, balance, shop_id from sales where created_at between '".$dyear."' and '".$date2."' and staff='".$shop."' and closed=1 group by transaction_id order by id desc";
   		...
   		$cust_sql="select * from sales where  created_at between '".$dyear."' and '".$date2."' and staff='".$shop."' and closed=1 group by transaction_id";
...
<?php /* echo CHtml::button('Checkout',
	...
	public function actionKitmenu()
	{
		...
		foreach($cat as $ca)
		{
			array_push($list,array('id'=>$ca['id'],'name'=>$ca['cat']));			
		}
		echo json_encode($list);
	}

	public function actionKitmenu2($id)
	{
		$items=Inventory::model()->findAll('category=:t',array(':t'=>$id));		
		$cut=array();
		foreach($items as $it)
		{
			array_push($cut,array('name'=>$it['name'],'price'=>$it['price'],'id'=>$it['id']));			
		}
	...
	{
		$dataa=Yii::app()->request->getPost();
		$data=json_decode(($dataa,true));
		$username=$data['username'];
		$password=$data['password'];
		$query=User::find('username=:u and password=:p',array(':u'=>$username,':p'=>SHA1($password)));
		if(count($query)==1)
		{
	...
	public function actionSalesapi()
	{
		ob_start();

		$posta=Yii::app()->request->getPost('sale');
		// $posta=$_POST;
		$post = json_decode($posta,true);
		$transaction=time();
		...
		{
			$new = new Sales();
			$new->item_name=$post[$i]['name'];
			$new->item_id=$post[$i]['id'];
			$new->qty=$post[$i]['quantity'];
			$new->unit_price=$post[$i]['price'];
			$new->total=0;
