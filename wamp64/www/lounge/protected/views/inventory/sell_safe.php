...
<input type="text" id="mi2" class="form-control" style="display:block; width:auto; margin:auto; margin-top:20px;  margin-bottom:20px;" placeholder="Type product name" onkeyup="findbar();">
...
<span style="height:auto; border:1px solid #ccc; width:98%; display:block; margin:auto; margin-top:20px;">
	...
	<span style="background-color:#ccc; padding:5px; text-align:center; display:block; margin-bottom:1px; ;">

<?php  echo CHtml::link(CHtml::encode($sale->total), array('Inventory/recent', 'id'=>$sale->transaction_id)); ?>
	</span>
	<?php } ?>
</span>
...
<!--input type="hidden" id="kdiscount" style="width:10px;" value="<?php echo $discount; ?>" -->
		...
		<table border="1" cellpadding="1" cellspacing="1" class="table">
			<tr id="b1" style="font-weight:bold; text-transform:capitalize; color:#fff; background-color:#333;"><td colspan="3">Customer Name: <input type="text" 
...
if($stock<$position->getQuantity()){ $flag=1; $color='red'; }
 ...
 <?php for($x=1; $x<1001; $x++){ ?>
  ...
  <option value="return">Return</option>
			...
			<?php }else{ ?>
			<tr style="font-weight:bold; background-color:#ccc;"><td style=""></td><td><input type="button" class="btn btn-primary" value="Get Change" onclick="getchange();" id="change2" style="margin-left:0px;"></td>
				<td></td><td>
				<select style="width:130px;" onchange="pay();" id="ptype">
  <option value=" ">Payment Type</option>
  <option value="cash">Cash</option>
